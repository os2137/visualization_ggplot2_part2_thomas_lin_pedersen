...
```{r}
# making the y-axis scale the same
...
```
We will get the same results as above by using the number as well instead of A, B, C 
...
p1 + p2 + p3 + plot_layout(design = layout)
```
We can also use the combo of number and letters
...
```{r}
# User comment: David Rengel: for those with pbs with gganimate, try installing gifski and av packages. THen Reboot RStudio
...
```{r}
?animate
# you can specify the duration  and fps (frame per second) arguments of the animate function. Different type of render function is also there (magic reder, avrender? etc). You can also have pause at the start, pause at end, remind etc. 
```

You can also have transition filter for your animation and you can also add layers gradually so there are lot of possibilities there. 
...
How to remove the label overlap with other points. 
```{r}
mtcars2 <- mtcars
mtcars2$label <- rownames(mtcars2)
points_to_label <- sample(nrow(mtcars), 10)
...
```{r}
# removing rounding of the corner
ggplot(mtcars, aes(x = disp, y = mpg)) +
  geom_point() + 
  geom_mark_rect(aes(filter = gear == 4,
...
ggplot(mtcars, aes(x = disp, y = mpg)) +
  ...
  geom_mark_circle(aes(filter = gear == 4,
                        label = '4 gear cars'))
```
One more thing you can do is to clip stuff outside of the panel 
```{r}
ggplot(mtcars, aes(x = disp, y = mpg)) +
  geom_point() + 
  geom_mark_circle(aes(filter = gear == 4,
...
```
The last geom is called hull 
```{r}
library(concaveman)
ggplot(mtcars, aes(x = disp, y = mpg)) +
  geom_point() + 
  geom_mark_hull(aes(filter = gear == 4,
                        label = '4 gear cars'), concavity = 2)

```
We can also use the overlapping 
...
If you dont need the label and you dont need the markings then, may be nonsensical in this case, but just to shwo that you can have one without the other. 
```{r}
ggplot(mtcars, aes(x = disp, y = mpg)) +
  geom_point() + 
  geom_mark_hull(aes(group = factor(gear), label = gear), fill = NA, color = NA)
```
One other package we should look at is 'gghilight' which is not getting discussed in this workshop. You can highlight/make this transparent/mute if certain conditions are not fulfilled. 
ggforce along with gghighlight is fantastic to draw attention to your readers. All of these can create publcation worthy figures without going into third party programs like adobe illustrator where you lose the reproducibility element. 
### Networks
...
```
We cannot if there is a cluster by simply looking at the network graph. 
